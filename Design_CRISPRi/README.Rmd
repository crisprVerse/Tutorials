---
title: "Using crisprDesign to design gRNAs for CRISPRi"
output: 
  github_document:
    toc: true
...
This tutorial will demonstrate how to use `crisprDesign` to design gRNAs for CRISPR interference (CRISPRi). Specifically, it will target the human KRAS gene and use the SpCas9 nuclease, however, much of the same process can be applied to any genomic target(s) and with any CRISPR nuclease.
...
and includes a column `dist_to_tss` that gives the distance in nucleotides between the TSS position and the PAM site of the gRNA. For CRISPRi, we recommend targeting the region 25-75bp region downstream of the TSS for optimal inhibition; see @sanson2018optimized for 
...
To demonstrate CRISPRi design, we will design gRNAs to inhibit expression of the human KRAS 
...
containing 500bp immediately downstream of the TSS. We first need to retrieve 
...
nonspecific to CRISPRi, for which we refer the reader to the
...
As with all CRISPR applications, potential off-targets effects are an important concern in 
assessing gRNA quality. While this concern is somewhat moderated for CRISPRi, since the dead CRISPR nuclease does not make DSBs, we should be aware of off-targets 
...
for up to 2 mismatches and increase the size of our `tss_window` (which defines the promoter region when searching for off-targets) to err 
on the safe side. 
...
(Users should change this to the filepath of their local bowtie index file).
...
You can see that the column `score_crisprai` was added to the `GuideSet`.
