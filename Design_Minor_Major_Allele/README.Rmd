...
The following examples use the reference, and variations of the reference,
genome sequence for Homo sapiens (UCSC version hg38, based on GRCh38.p12). 
For documentation on each `BSgenome` object, pass the object to `help()` or 
preceed it with `?`, as shown below:
...
Indeed, given that it was historically constructed from a small set of human genomes, it contains minor alleles that were common across this set of human
genomes.
